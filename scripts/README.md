...
validate_eval_regression_v3_1.py      校验 v3.1 稳定Gold引用及当前数据库解析结果
migrate_eval_v3_stable_refs.py        将v3易变Chunk ID迁移为文档SHA/ordinal/Qdrant UUID稳定引用
resolve_eval_gold_refs.py             用运行时数据库将稳定Gold引用解析为当前Chunk ID
...
```text
backup-demo.ps1                     在线备份SQLite、Qdrant集合快照并生成SHA-256清单
create_demo_backup.py              在API容器内执行一致性备份的Python实现
```
...
备份写入 `knowledge/backups/<时间戳>/`。`knowledge/raw/` 是宿主机上的不可变原件，
不会在每次备份中重复复制；迁移机器时应将 `knowledge/raw/` 与备份目录一起保存。
